...
def test_gcn2_conv(device):
    ...
    if torch_geometric.typing.WITH_TORCH_SPARSE:
        ...
        assert torch.allclose(conv(x, x_0, adj3.t()), out1)
        assert torch.allclose(conv(x, x_0, adj4.t()), out2)

    if is_full_test():
        jit = torch.jit.script(conv)
        assert torch.allclose(jit(x, x_0, edge_index), out1)
        assert torch.allclose(jit(x, x_0, edge_index, value), out2)

        if torch_geometric.typing.WITH_TORCH_SPARSE:
            assert torch.allclose(jit(x, x_0, adj3.t()), out1)
            assert torch.allclose(jit(x, x_0, adj4.t()), out2)

    conv.cached = True
    conv(x, x_0, edge_index)
    assert conv._cached_edge_index is not None
    assert torch.allclose(conv(x, x_0, edge_index), out1)
    assert torch.allclose(conv(x, x_0, adj1.t()), out1)
