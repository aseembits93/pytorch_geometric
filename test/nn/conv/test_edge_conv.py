...
def test_edge_conv_conv(device):
    x1 = torch.randn(4, 16)
    x2 = torch.randn(2, 16)
    ...
    assert torch.allclose(conv((x1, x1), edge_index), out)
    assert torch.allclose(conv(x1, adj1.t()), out)
    assert torch.allclose(conv((x1, x1), adj1.t()), out)

    if torch_geometric.typing.WITH_TORCH_SPARSE:
        adj2 = SparseTensor.from_edge_index(edge_index, sparse_sizes=(4, 4))
        assert torch.allclose(conv(x1, adj2.t()), out)
        assert torch.allclose(conv((x1, x1), adj2.t()), out)

    if is_full_test():
        jit = torch.jit.script(conv)
        assert torch.allclose(jit(x1, edge_index), out)
        assert torch.allclose(jit((x1, x1), edge_index), out)

        if torch_geometric.typing.WITH_TORCH_SPARSE:
            assert torch.allclose(jit(x1, adj2.t()), out)
            assert torch.allclose(jit((x1, x1), adj2.t()), out)
...
def test_dynamic_edge_conv(device):
    x1 = torch.randn(8, 16)
    x2 = torch.randn(4, 16)
