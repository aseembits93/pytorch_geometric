...
def linear_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    """
    # sum over key sequence dimension once and reuse
    k_sum = k.sum(dim=-2)
    # compute denominator and its reciprocal (keep last dim for broadcasting)
    D_inv = (q @ k_sum.unsqueeze(-1)).reciprocal()
    # compute key-value and then q @ (k^T v)
    kv = k.transpose(-2, -1) @ v
    qkv = q @ kv
    # elementwise multiply using broadcasting instead of einsum
    out = qkv * D_inv
    return out
